...
description: >
  ...
  Phase 4 (optional): JSONL data export as {codename}_box.cards — lossless serialization of Phase 3 for the Card Box Viewer; preserves Set→Card hierarchy with semantically-named palette.
...
| **Card Box Data** | `{codename}_box.cards` | — (consumed by Card Box Viewer) | One JSON object per line; line 1 = metadata, lines 2..N = cards |
...
| **4 — JSONL Export** *(optional)* | Serialise Phase 3 Card Box for the Card Box Viewer | `{codename}_box.cards` |
...
## Phase 4 — JSONL Data Export (`{codename}_box.cards`) *(Optional)*
...
| 4 (monolingual) | `{codename}_box.cards` |
| 4 (bilingual, both languages) | `{codename}_box.en.cards` and `{codename}_box.{l2}.cards` |
